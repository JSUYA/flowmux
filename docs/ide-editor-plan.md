...
- 여러 문서 buffer 유지와 file browser 기반 전환
- 저장, 모두 저장, 다른 이름으로 저장
...
- 파일 클릭과 기본 애플리케이션 열기: `crates/flowmux/src/ui/file_browser.rs`
...
파일 뷰어의 기본 파일 열기를 `OpenFileInEditor` 명령으로 연결하되, 이미지·영상·PDF·binary와 `Open Externally` 동작은 보존한다.
...
- tab 제목과 conflict 상태 표시의 모든 상태를 색만으로 전달하지 않는다.
...
- pane 전체를 채우는 단일 편집 화면과 필요할 때만 보이는 저장 상태 표시 완성
