...
P0 ("contract and agent usability") is implemented:

- P0-1 / P0-2 — `flowmux browser <op>` namespace plus the previously
  CLI-missing verbs (`dblclick`, `hover`, `focus`, `blur`, `check`,
  `uncheck`, `is-visible`, `is-enabled`, `is-checked`, `count`). The old
  hyphenated `browser-*` commands stay as hidden compatibility aliases.
- P0-3 — single non-mutating `RefStore` snapshot path; the DOM-stamping
  path is removed and refs invalidate on navigation.
- P0-4 — `flowmux identify` and `flowmux capabilities`.
- P0-5 — `flowmux tree`, `workspace current` / `workspace focus`,
  `focus-pane` / `close-pane`, `focus-tab` / `close-tab`. Close verbs refuse
