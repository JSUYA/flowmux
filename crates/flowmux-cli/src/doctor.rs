...
async fn section_daemon(socket: Option<PathBuf>) -> Section {
    ...
    let mut entries = vec![Entry {
        name: "socket".into(),
        // Informational either way — a missing socket only means the GUI
        // isn't running, which the Ping below reports as Warn.
        status: Status::Info,
    ...
    fn render_text_with_color_wraps_status_badges_in_sgr() {
        let report = Report {
            sections: vec![Section {
                title: "Status colours".into(),
                entries: vec![
                    Entry {
                        name: "installed row".into(),
                        status: Status::Ok,
                        detail: "ready".into(),
                    },
                    Entry {
                        name: "repair row".into(),
                        status: Status::NeedsFix,
                        detail: "missing".into(),
                    },
                ],
