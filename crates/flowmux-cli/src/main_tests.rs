...
fn read_screen_parses_pane_arg_and_env_fallback() {
    let pane = PaneId::new();
    // Explicit pane: arg.
    let cli = Cli::try_parse_from(["flowmuxctl", "read-screen", &format!("pane:{pane}")]).unwrap();
    assert!(matches!(
        build_request(cli.cmd).unwrap(),
...
fn tmux_style_aliases_map_to_existing_ipc_requests() {
    ...
    let cli =
        Cli::try_parse_from(["flowmuxctl", "resize-pane", &pane_arg, "--ratio", "0.6"]).unwrap();
    assert!(matches!(
        build_request(cli.cmd).unwrap(),
...
fn focus_pane_and_close_pane_parse_and_map() {
    let pane = PaneId::new();
    let cli = Cli::try_parse_from(["flowmuxctl", "focus-pane", &format!("pane:{pane}")]).unwrap();
    assert!(matches!(
        build_request(cli.cmd).unwrap(),
        Request::PaneFocus { pane: got } if got == pane
    ));
...
fn workspace_focus_parses_and_maps_to_request() {
    ...
    let cli = Cli::try_parse_from(["flowmuxctl", "workspace", "focus", &ws.to_string()]).unwrap();
    assert!(matches!(
        build_request(cli.cmd).unwrap(),
...
fn local_only_commands_parse_to_local_variants() {
    ...
    let import =
        Cli::try_parse_from(["flowmuxctl", "theme", "import", "/tmp/flowmux-theme.toml"]).unwrap();
    assert!(matches!(
        import.cmd,
        Cmd::Theme {
