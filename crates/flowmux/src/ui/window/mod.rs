...
impl WindowController {
    ...
    pub async fn dispatch(&self, cmd: GtkCommand) {
        match cmd {
            ...
            command => self.dispatch_non_browser_command(command).await,
        }
    }

    async fn dispatch_non_browser_command(&self, cmd: GtkCommand) {
        match cmd {
            ...
            other => unreachable!("browser command bypassed browser router: {other:?}"),
        }
    }
