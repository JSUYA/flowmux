...
fn workspace_agent_blocks_sort_by_status_then_recent_focus() {
    ...
    let ws = workspace_with_agent_leaves(vec![
        ...
        (
            blocked,
            terminal_surface_with_agent("blocked", "/tmp/blocked", "claude", AgentStatus::Blocked),
        ),
        (
            working_recent,
            terminal_surface_with_agent("recent", "/tmp/recent", "opencode", AgentStatus::Working),
        ),
    ]);
...
fn agent_bar_model_reflects_agent_tab_pane_and_workspace_removal() {
    ...
    let surface_b = terminal_surface_with_agent("b", "/tmp/b", "opencode", AgentStatus::Blocked);
